...
setwd(dir="/Users/Florian/Desktop/Scripts-Evolution-CF-proteome-allocation/Plot-function")
...
#1.Adaptive dynamics - effect of the length of the pathway (size=5+5)
...
Etot_back_set<-c(5.5*10^-3,9e-3,9.25e-3,9.5e-3,9.75e-3)#5.5*10^-3,8.5e-3,9e-3,9.5e-3,10e-3)#other protein concentration
...
#Variables
Etot_var<-10^seq(-4.5,-3.5,length=N_reso)
...
Enz=1
for(po in 1:length(Etot_back_set)){
  Etot_back<-Etot_back_set[po]
  alpha=alpha_set[1]
  beta=beta_set[1]
  for (p in 1:length(eta_set)){
    ...
    for (i in 1:N_reso){
      ...
      while((abs(Phi_fit-Phi_eq)/Phi_eq>10^-8) && N_eq>1){
        ...
        S<-nleqslv(c(1e-3,1e-3),transport,jac=NULL)$x
        Sin<-S[2]
        P1<-c()
        ...
        P2_sol<-nleqslv(c(1e-5,1e-5),pathway2,jac=NULL)$x
        P2[1]<-P2_sol[1]
...
for(po in 1:length(Etot_back_set)){
  Pox_i<-as.character(Etot_back_set[po])
...
#A.intermediate rate of environment replenishment (alpha=beta=1e-4)
...
for (p in 1:length(eta_set)){
  ...
  for (pe in 1:length(Etot_back_set)){
    Etot_back<-Etot_back_set[pe]
    Etot_conc=Conc1_opt_DA_E0E1comb[[as.character(Etot_back)]][p]*2# Double so that investing half in each sub-pathway coincides with the optimum when considered as a single integrated pathway
    P_p2=P_p2_set[1]
    eta_p=eta+P_p2*S_c/V_c
    PO1<-5
    PO2<-5
...
pal<-list(palet)
Pop_DA1<-Pop_DA
multiplePlot("","","Ebas (mM)=","",c(""),c("5.5","9","9.25","9.5","9.75")
             ,ncol=800,f_var_print,f_var_print,list_fit_inv_S40ratvar_Y1_eta1[[1]],
             abs="res. (% 1st pathway)",ord="mut. (% 1st pathway)",scale=c(-999,999),palette=pal,cextext=1.5,TEXT_to_Add=addtxt,
             image=TRUE,pcex=1,subcex=1,labcex=1.5,axcex=0.75,globcex=0.5,legcex=1,contourlab=TRUE,meth="edge",contcex=0.5,colorkey="COMMON")

setwd(dir="~")
setwd(dir="/Users/Florian/Desktop/Manuscripts/Revision-MCF")
dev.print(device = jpeg, file = "Coexistence-PIP-etamax-Pathlength10.jpeg", width = 1500*6,height=300*6,res=600,type="cairo")
dev.off()

#2.Adaptive dynamics - effect of the length of the pathway (size=10+10)
...
N_reso=200#Resolution used for strategies (in terms of concentration)
Etot_back_set<-c(5.5*10^-3,6e-3,6.5e-3,7e-3,7.5e-3)#5.5*10^-3,8.5e-3,9e-3,9.5e-3,10e-3)#other protein concentration
E_basal<-3*10^-3#scaling factor
...
pal<-list(palet)
Pop_DA1<-Pop_DA
multiplePlot("","","Ebas (mM)=","",c(""),c("5.5","6","6.5","7","7.5")
             ,ncol=800,f_var_print,f_var_print,list_fit_inv_S40ratvar_Y1_eta1[[1]],
             abs="res. (% 1st pathway)",ord="mut. (% 1st pathway)",scale=c(-999,999),palette=pal,cextext=1.5,TEXT_to_Add=addtxt,
             image=TRUE,pcex=1,subcex=1,labcex=1.5,axcex=0.75,globcex=0.5,legcex=1,contourlab=TRUE,meth="edge",contcex=0.5,colorkey="COMMON")
...
setwd(dir="~")
setwd(dir="/Users/Florian/Desktop/Manuscripts/Revision-MCF")
dev.print(device = jpeg, file = "Coexistence-PIP-etamax-Pathlength20.jpeg", width = 1500*6,height=300*6,res=600,type="cairo")
dev.off()


#2.Adaptive dynamics - effect of the length of the pathway (size=20+20,slightly higher diffusive constraint)
...
N_reso=200#Resolution used for strategies (in terms of concentration)
Etot_back_set<-c(4*10^-3,4.25e-3,4.5e-3,4.75e-3,5e-3)#5.5*10^-3,8.5e-3,9e-3,9.5e-3,10e-3)#other protein concentration
E_basal<-2.75*10^-3#scaling factor
...
pal<-list(palet)
Pop_DA1<-Pop_DA
multiplePlot("","","Ebas (mM)=","",c(""),c("4","4.25","4.5","4.75","5")
             ,ncol=800,f_var_print,f_var_print,list_fit_inv_S40ratvar_Y1_eta1[[1]],
             abs="res. (% 1st pathway)",ord="mut. (% 1st pathway)",scale=c(-999,999),palette=pal,cextext=1.5,TEXT_to_Add=addtxt,
             image=TRUE,pcex=1,subcex=1,labcex=1.5,axcex=0.75,globcex=0.5,legcex=1,contourlab=TRUE,meth="edge",contcex=0.5,colorkey="COMMON")
...
setwd(dir="~")
setwd(dir="/Users/Florian/Desktop/Manuscripts/Revision-MCF")
dev.print(device = jpeg, file = "Coexistence-PIP-etamax-Pathlength40-higherdiffconstr.jpeg", width = 1500*6,height=300*6,res=600,type="cairo")
dev.off()
