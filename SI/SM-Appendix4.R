...
Enz=1
for(po in 1:length(Phi_eq_set)){
  Phi_eq=Phi_eq_set[po]
  alpha=alpha_set[1]
  beta=beta_set[1]
...
for(po in 1:length(Phi_eq_set)){
  Pox_i<-as.character(Phi_eq_set[po])
...
for (p in 1:length(eta_set)){
  ...
  for (pe in 1:length(P_p2_set)){
    Etot_conc=Conc1_opt_DA_E0E1comb[[as.character(Phi_eq)]][p]*2# Double so that investing half in each sub-pathway coincides with the optimum when considered as a single integrated pathway
    P_p2=P_p2_set[pe]
    eta_p=eta+P_p2*S_c/V_c
    PO1<-5
    PO2<-5
...
dev.print(device = jpeg, file = "Coexistence-PIP-etamax-Philow.jpeg", , width = 1500*6,height=300*6,res=600,type="cairo")
dev.off()
...
setwd(dir="~")
setwd(dir="/Users/Florian/Desktop/Manuscripts/Revision-MCF")
dev.print(device = jpeg, file = "Coexistence-PIP-etamax-Phihigh.jpeg", , width = 1500*6,height=300*6,res=600,type="cairo")
dev.off()
