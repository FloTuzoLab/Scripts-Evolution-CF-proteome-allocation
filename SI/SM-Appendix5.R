...
Phi_eq=10^-4
r_c_set=c(10^-5.5,10^-5,10^-4.5)#dm
#S_c=4*pi*r_c^2
  ...
  for (p in 1:length(eta_set)){
    ...
    for (i in 1:N_reso){
      ...
      while((abs(Phi_fit-Phi_eq)/Phi_eq>10^-8) && N_eq>1){
        ...
        P2_sol<-nleqslv(c(1e-3,1e-3),pathway2,jac=NULL)$x
        P2[1]<-P2_sol[1]
...
for (pe in 1:length(P_p2_set)){
  ...
  for (rat in 1:length(r_c_set)){
    r_c<-r_c_set[rat]
    S_c=4*pi*r_c^2
    V_c=4/3*pi*r_c^3#dm^3
    Etot_conc=Conc1_opt_DA_E0E1comb[[1]][1]*2#No difference on the optimum concentration
    P_p2=P_p2_set[pe]
    eta_p=eta+P_p2*S_c/V_c
    PO1<-5
    PO2<-5
...
pal<-list(palet)
Pop_DA1<-Pop_DA
multiplePlot("","","r=","",c(""),c("1e-5.5","1e-5","1e-4.5")
...
dev.print(device = jpeg, file = "Coexistence-PIP-6-RcellSens.jpeg", , width = 1050*6,height=300*6,res=600,type="cairo")
dev.off()
