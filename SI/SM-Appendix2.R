...
for(po in 1:length(PayOff_set)){
  ...
  for (to in 1:length(Tox_set)){
    ...
    while(list_fit_inv_S40_noperm[[to+length(Tox_set)*(po-1)]][i,i]==-100 && i<N_reso){
      i=i+1
      if(i==N_reso){
        Conc1_opt_DA_E0E1comb[[Pox_i]][to]<-10^-10
      }
    }
    S_opt=0
    while (i<N_reso-1){
      if(list_fit_inv_S40_noperm[[to+length(Tox_set)*(po-1)]][i,i+1]<0 && list_fit_inv_S40_noperm[[to+length(Tox_set)*(po-1)]][i,i-1]<0){
        ...
        if(list_fit_inv_S40_noperm[[to+length(Tox_set)*(po-1)]][i+1,i]<0 && list_fit_inv_S40_noperm[[To+length(Tox_set)*(po-1)]][i,i+1]>0){
          Conc1_opt_DA_E0E1comb[[Pox_i]][to]=10^-4
          i=i+1
          break
        }
        else{
          Conc1_opt_DA_E0E1comb[[Pox_i]][to]<-0
          break
        }
      }
      else{
...
for (to in 1:length(Tox_set)){
  list_fit_inv_coex_5[[to]]<-list()
  list_fit_inv_coex_5_rec[[to]]<-list()
  Tox<-Tox_set[to]
  PayOff<-PayOff_set[1]
  ...
  for (pe in 1:length(P_p2_set)){
    ...
    for (i in 1:N_reso){
      ...
      while((abs(Phi_fit-Phi_eq)/Phi_eq>10^-8) && N_eq>1){
        ...
        Phi_fit<-((Phi1r*PO1+Phi2r*PO2)-prot_cost*(Etot_conc_r+Etot_back))*Tox/(Tox+Sin+sum(P1)+sum(P2))#-VTm*10^-1.5
        ##Algorithm to simulate the population size at demographic equilibrium for the resident strategy
        if(count<10){
          if(Phi_fit>Phi_eq){
            N_eq=(N_eq*2)
      ...
      Phi_res<-((Phi1r*PO1+Phi2r*PO2)-prot_cost*(Etot_conc_r+Etot_back))*Tox/(Tox+Sin_res+sum(P1_res)+sum(P2_res))#-VTm*10^-1.5
      for (j in 1:N_reso){
        #VTm=VTm_max*f_var[j]
        ...
        if(as.numeric((Phi1*PO1+Phi2*PO2)-prot_cost*((E_tot_conc1*Path1_size+E_tot_conc2*(Path_size-Path1_size))+(Etot_back)))>0){
          P_Phi_eq=as.numeric(((Phi1*PO1+Phi2*PO2)-prot_cost*(Etot_conc_m+Etot_back))*Tox/(Tox+sum(S_conc_eq)+sum(P1_conc_eq)+sum(P2_conc_eq)))#
        }
        else{
          P_Phi_eq=0
        }
    ...
    list_fit_inv_coex_5[[to]][[pe]]<-fit_inv
    list_fit_inv_coex_5_rec[[to]][[pe]]<-fit_inv_rec
  }
}
...
multiplePlot("","","","",c(""),c("P=1e-5","P=1e-4")
             ,ncol=2,f_var_print,f_var_print,list_fit_inv_coex_5[[2]],
             abs="resident (logEtot1)",ord="mutant (logEtot1)",scale=c(-900,900),lev=c(0),palette=pal,cextext=2,TEXT_to_Add=addtxt,
...
for (to in 1:length(Tox_set)){
  list_fit_inv_coex_5_area[[to]]<-list()
  for (pe in 1:length(P_p2_set)){
    for (i in 1:N_reso){
      for (j in 1:N_reso){
        print(paste(to,pe,i))
        if(abs(list_fit_inv_coex_5[[to]][[pe]][i,j])<10^-5 && abs(list_fit_inv_coex_5_rec[[to]][[pe]][i,j])<10^-5){
          fit_inv_coex[i,j]=0
        }
        else if(list_fit_inv_coex_5[[to]][[pe]][i,j]==-1000 && list_fit_inv_coex_5_rec[[to]][[pe]][i,j]==-1000){
          fit_inv_coex[i,j]=-100
        }
        else if(list_fit_inv_coex_5_rec[[to]][[pe]][i,j]==-1000 && list_fit_inv_coex_5[[to]][[pe]][i,j]>0){
          fit_inv_coex[i,j]=1
        }
        else if(list_fit_inv_coex_5[[to]][[pe]][i,j]==-1000 && list_fit_inv_coex_5_rec[[to]][[pe]][i,j]>0){
          fit_inv_coex[i,j]=1
        }
        else if(list_fit_inv_coex_5[[to]][[pe]][i,j]>0 && list_fit_inv_coex_5_rec[[to]][[pe]][i,j]>0){
          fit_inv_coex[i,j]=1
        }
        else{
          fit_inv_coex[i,j]=-1
    ...
    list_fit_inv_coex_5_area[[to]][[pe]]<-fit_inv_coex
  }
}
...
multiplePlot("","","","",c(""),c("P=1e-5","P=1e-4")
             ,ncol=2,f_var_print,f_var_print,list_fit_inv_coex_5_area[[2]],
...
for (to in 1:length(Tox_set)){
  #VTm<-VTm_max
  Etot_conc=Conc1_opt_DA_E0E1comb[[as.character(PayOff)]][to]*2
  Path1_size=P1s_func(Pts=Path_size,ratio1_2=ratio_12)
  Tox<-Tox_set[to]
  for (pe in 1:length(P_p2_set)){
    P_p2=P_p2_set[pe]
    ...
    while(r1<(N_reso)){
      ...
      while(r2<(N_reso)){
        ...
        #r2=21
        print(paste(r1,r2))
        if(list_fit_inv_coex_5_area[[to]][[pe]][r1,r2]==1){
          #Etot_conc=2*Conc_opt_DA_Degperm_S40_noperm_eta1[1]
          #if(r1==1 || r2==1){
          #  VTm<-VTm_max*c(f_var[r1],f_var[r2])+VTm_max/10^6
          #}
          #else{
          ...
          Phi_fit<-c(0,0)
          P2_out=1
          while((abs(Phi_fit[1]-Phi_eq)/Phi_eq>10^-4 || (abs(Phi_fit[2]-Phi_eq)/Phi_eq>10^-4)) && (N_eq1>1 && N_eq2>1)){
            transport<-function(x){
              t1 <- alpha-beta*x[3]-V_c/V_env*(N_eq1*VTm*(x[3]-x[1])/(KT+(x[1]+x[3])+x[1]*x[3]/KT)+N_eq2*VTm*(x[3]-x[2])/(KT+(x[2]+x[3])+x[2]*x[3]/KT))
              type=1
              t2<-c()
              while(type<3){
                t2[type] <- VTm*(x[3]-x[type])/(KT+(x[type]+x[3])+x[3]*x[type]/KT)-Vm0r[type]*x[type]/(x[type]+KMr[type])
                type=type+1
              }
              c(t1,t2)
            }
            S<-nleqslv(c(1e-8,1e-6,1e-6),transport,jac=NULL)$x
            ...
            P2_sol<-nleqslv(c(1e-6,1e-6,1e-6),pathway2,jac=NULL)$x
            P2_1<-c()
            P2_2<-c()
            P2_1[1]<-P2_sol[1]
            P2_2[1]<-P2_sol[2]
            ...
            for (p2 in 1:(Path_size-Path1_size)){
              ...
              Phi2r[2]=Phi2r[2]+(Vm2r[2]*P2_2[p2]/(P2_2[p2]+KMr[2]))/(Path_size/2)
            }
            
            Phi_fit[1]<-((Phi1r[1]*PO1+Phi2r[1]*PO2)-prot_cost*(Etot1r1*Path1_size+Etot2r1*(Path_size-Path1_size)+Etot_back))*Tox/(Tox+Sin[1]+sum(P1_1)+sum(P2_1))
            Phi_fit[2]<-((Phi1r[2]*PO1+Phi2r[2]*PO2)-prot_cost*(Etot1r2*Path1_size+Etot2r2*(Path_size-Path1_size)+Etot_back))*Tox/(Tox+Sin[2]+sum(P1_2)+sum(P2_2))
            if(count<10){
              if(Phi_fit[1]>Phi_eq){
                N_eq1=N_eq1*2
              }
          ...
          for (p1 in 1:(Path1_size-1)){
            ...
            sumP1rr=sumP1rr+P1_conc_eq[[p1]]
          }
          for (p2 in 1:(Path_size-Path1_size)){
            Phi2rr=Phi2rr+(Vm2r*P2_conc_eq[[p2]]/(P2_conc_eq[[p2]]+KMr))/(Path_size/2)
            sumP2rr=sumP2rr+P2_conc_eq[[p2]]
          }
          Phirr<-((Phi1rr*PO1+Phi2rr*PO2)-prot_cost*(c(Etot1r1,Etot1r2)*Path1_size+c(Etot2r1,Etot2r2)*(Path_size-Path1_size)+(Etot_back)))*Tox/(Tox+sumP1rr+sumP2rr)
          ...
          for (rm in 1:max_rm){
            ...
            #if(r1==1 || r2==1){
            #  VTm=VTm_max*f_var[co_m]+VTm_max/10^6
            #}
            #else{
            ...
            if(as.numeric((Phi1*PO1+Phi2*PO2)-prot_cost*(E_tot_conc1*Path1_size+E_tot_conc2*(Path_size-Path1_size)+(Etot_back)))>0){
              P_Phi_eq=((Phi1*PO1+Phi2*PO2)-prot_cost*(E_tot_conc1*Path1_size+E_tot_conc2*(Path_size-Path1_size)+(Etot_back)))*Tox/(Tox+sum(S_conc_eq)+sum(P1_conc_eq)+sum(P2_conc_eq))
            }
            else{
              P_Phi_eq=0
            }
    ...
    Neq1_list[[pe+length(P_p2_set)*(to-1)]]<- Neq1_tab
    Neq2_list[[pe+length(P_p2_set)*(to-1)]]<- Neq2_tab
    list_fit_gradinv_P5[[pe+length(P_p2_set)*(to-1)]]<-fit_gradinv_P5
  }
}
  ...
  for (pe in 1:length(P_p2_set)){
    for (i in  1:N_reso){
      for (j in 1:N_reso){
        #print()
        print(paste(i,j))
        if(length(list_fit_gradinv_P5[[pe+length(P_p2_set)*(to-1)]][[i]][[j]])==1){
          fit_coAlinv_P5[i,j]=-1
        }
        else if(list_fit_inv_coex_5_area[[to]][[pe]][i,j]==-1){
          fit_coAlinv_P5[i,j]=-1
        }
        else if(length(list_fit_gradinv_P5[[pe+length(P_p2_set)*(to-1)]][[i]][[j]])==4){
          if(trueVec(list_fit_gradinv_P5[[pe+length(P_p2_set)*(to-1)]][[i]][[j]],c(1,1,1,1))){
            fit_coAlinv_P5[i,j]=-1#repell
          }
          else if(trueVec(list_fit_gradinv_P5[[pe+length(P_p2_set)*(to-1)]][[i]][[j]],c(-1,-1,-1,-1))){
    ...
    list_fit_coAlinv_P5[[pe+length(P_p2_set)*(to-1)]]<-fit_coAlinv_P5
  }
}

#TE PLOT FOR HIGH TOXICITY AND MAX PERMEABILITY 1E-4
dev.off()
...
for (to in 2:2){
  ...
  for (pe in 2:2){
    multiplePlot("","","","",c(""),c("")
                 ,ncol=2,f_var_print,f_var_print,list(list_fit_inv_coex_5_area[[to]][[2]]),
    ...
    while(i<N_reso+1){
      j=1
      while(j<N_reso+1){
        if(list_fit_coAlinv_P5[[pe+length(P_p2_set)*(to-1)]][i,j]==0){
    ...
    while(i<N_reso+1){
      ...
      while(j<N_reso+1){
        print(paste(i,j))
        if(list_fit_coAlinv_P5[[pe+length(P_p2_set)*(to-1)]][i,j]==-1){
        }
        if(list_fit_coAlinv_P5[[pe+length(P_p2_set)*(to-1)]][i,j]==1){
          arrows(f_var[i], f_var[j], f_var[i]-size, f_var[j], length=0.025, col="black", lwd=1)
        }
        if(list_fit_coAlinv_P5[[pe+length(P_p2_set)*(to-1)]][i,j]==2){
          arrows(f_var[i], f_var[j], f_var[i]+size, f_var[j], length=0.025, col="black", lwd=1)
        }
        if(list_fit_coAlinv_P5[[pe+length(P_p2_set)*(to-1)]][i,j]==3){
          arrows(f_var[i], f_var[j], f_var[i], f_var[j]-size, length=0.025, col="black", lwd=1)
        }
        if(list_fit_coAlinv_P5[[pe+length(P_p2_set)*(to-1)]][i,j]==4){
          arrows(f_var[i], f_var[j], f_var[i], f_var[j]+size, length=0.025, col="black", lwd=1)
        }
        if(list_fit_coAlinv_P5[[pe+length(P_p2_set)*(to-1)]][i,j]==5){#lr
          arrows(f_var[i], f_var[j], f_var[i]-size, f_var[j], length=0.025, col="black", lwd=1)
          arrows(f_var[i], f_var[j], f_var[i]+size, f_var[j], length=0.025, col="black", lwd=1)
        }
        if(list_fit_coAlinv_P5[[pe+length(P_p2_set)*(to-1)]][i,j]==6){#ld
          arrows(f_var[i], f_var[j], f_var[i]-size, f_var[j], length=0.025, col="black", lwd=1)
          arrows(f_var[i], f_var[j], f_var[i], f_var[j]-size, length=0.025, col="black", lwd=1)
        }
        if(list_fit_coAlinv_P5[[pe+length(P_p2_set)*(to-1)]][i,j]==7){#lt
          arrows(f_var[i], f_var[j], f_var[i]-size, f_var[j], length=0.025, col="black", lwd=1)
          arrows(f_var[i], f_var[j], f_var[i], f_var[j]+size, length=0.025, col="black", lwd=1)
        }
        if(list_fit_coAlinv_P5[[pe+length(P_p2_set)*(to-1)]][i,j]==8){#rd
          arrows(f_var[i], f_var[j], f_var[i]+size, f_var[j], length=0.025, col="black", lwd=1)
          arrows(f_var[i], f_var[j], f_var[i], f_var[j]-size, length=0.025, col="black", lwd=1)
        }
        if(list_fit_coAlinv_P5[[pe+length(P_p2_set)*(to-1)]][i,j]==9){#rt
          arrows(f_var[i], f_var[j], f_var[i]+size, f_var[j], length=0.025, col="black", lwd=1)
          arrows(f_var[i], f_var[j], f_var[i], f_var[j]+size, length=0.025, col="black", lwd=1)
        }
        if(list_fit_coAlinv_P5[[pe+length(P_p2_set)*(to-1)]][i,j]==10){#td
          arrows(f_var[i], f_var[j], f_var[i], f_var[j]-size, length=0.025, col="black", lwd=1)
          arrows(f_var[i], f_var[j], f_var[i], f_var[j]+size, length=0.025, col="black", lwd=1)
        }
        if(list_fit_coAlinv_P5[[pe+length(P_p2_set)*(to-1)]][i,j]==11){#lrd
          arrows(f_var[i], f_var[j], f_var[i]-size, f_var[j], length=0.025, col="black", lwd=1)
          arrows(f_var[i], f_var[j], f_var[i]+size, f_var[j], length=0.025, col="black", lwd=1)
          arrows(f_var[i], f_var[j], f_var[i], f_var[j]-size, length=0.025, col="black", lwd=1)
        }
        if(list_fit_coAlinv_P5[[pe+length(P_p2_set)*(to-1)]][i,j]==12){#ldt
          arrows(f_var[i], f_var[j], f_var[i]-size, f_var[j], length=0.025, col="black", lwd=1)
          arrows(f_var[i], f_var[j], f_var[i], f_var[j]-size, length=0.025, col="black", lwd=1)
          arrows(f_var[i], f_var[j], f_var[i], f_var[j]+size, length=0.025, col="black", lwd=1)
        }
        if(list_fit_coAlinv_P5[[pe+length(P_p2_set)*(to-1)]][i,j]==13){#lrt
          arrows(f_var[i], f_var[j], f_var[i]-size, f_var[j], length=0.025, col="black", lwd=1)
          arrows(f_var[i], f_var[j], f_var[i]+size, f_var[j], length=0.025, col="black", lwd=1)
          arrows(f_var[i], f_var[j], f_var[i], f_var[j]+size, length=0.025, col="black", lwd=1)
        }
        if(list_fit_coAlinv_P5[[pe+length(P_p2_set)*(to-1)]][i,j]==14){#rtd
          arrows(f_var[i], f_var[j], f_var[i]+size, f_var[j], length=0.025, col="black", lwd=1)
          arrows(f_var[i], f_var[j], f_var[i], f_var[j]-size, length=0.025, col="black", lwd=1)
          arrows(f_var[i], f_var[j], f_var[i], f_var[j]+size, length=0.025, col="black", lwd=1)
        }
...
points(x=0.13,y=0.13,,col="black",pch=13,cex=3.5)
legend("right",title="Singular Strategy",legend=c("Loc. stable","Glob. stable","Glob. instable"),bty="y",pch=c(20,19,13,19),col=c(1,"green",1,"blue"),ncol=1,cex=1.5)
setwd(dir="~")
setwd(dir="/Users/florianlabourel/Desktop/Ongoing-projects/cross-feeding/Proteome allocation/Draft/SM/Figures-February")
dev.print(device = jpeg, file = "Coex-tox-4.jpeg", width = 575*6,height=500*6,res=600,type="cairo")
dev.off()   
