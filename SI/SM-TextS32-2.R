...
###1.Influence of degradation rate and enzyme efficiencies on the allocation between subpathways (+influence of transporters)
...
VTm=1e-3#M/s
KT=1e-2#M
kf=10^6##set to a moderately high value
kcat=10^2#/s##set to a moderately high value
kr=kcat#
...
#Parameters
eta_set=c(10^-3,10^-2.5,10^-2,10^-1.5)
...
for (p in 1:length(eta_set)){
  ...
  for (pe in 1:length(P_p2_set)){
    if(eta==10^-1.5){
      Etot_conc=Conc1_opt_DA_E0E1comb[[as.character(PayOff)]][3]*2
    }
    else{
...
multiplePlot("","","","",c("","",""),c("","","")
             ...
             abs="res. (% 1st pathway)",ord="mut. (% 1st pathway)",scale=c(-999,999),lev=c(0),palette=pal,cextext=1.5,TEXT_to_Add=addtxt,
             image=TRUE,pcex=1,subcex=1,labcex=1.5,axcex=0.75,globcex=0.5,legcex=1,contourlab=TRUE,meth="edge",contcex=0.5,colorkey="COMMON")
...
for (p in 1:length(eta_set)){
  ...
  for (pe in 1:length(P_p2_set)){
    i=1
    while(list_fit_inv_S40ratvar_Y1_eta1[[p]][[pe]][i,i]==-1000 && i<N_reso){
      i=i+1
    }
    S_opt=0
    print(i)
...
plot(Rep_opt_DA_Deg_S40_perm[[1]]~log10(P_p2_set),col="white",pch=15,ylim=c(-0,1),xlab="log10(Permeability rate)",ylab=expression(paste(delta," (% 1st pathway)")))
points(Rep_opt_DA_Deg_S40_perm[[1]][1:7]~log10(P_p2_set)[1:7],col="dark grey",pch=19,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[1]][1:7]~log10(P_p2_set)[1:7],col="black",pch=16,cex=1)
points(Rep_opt_DA_Deg_S40_perm[[1]][8:9]~log10(P_p2_set)[8:9],col="dark grey",pch=1,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[1]][8:9]~log10(P_p2_set)[8:9],col="black",pch=16,cex=1)
points(Rep_opt_DA_Deg_S40_perm[[2]][1:7]~log10(P_p2_set)[1:7],col="light green",pch=19,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[2]][1:7]~log10(P_p2_set)[1:7],col="dark green",pch=16,cex=1)
points(Rep_opt_DA_Deg_S40_perm[[2]][8:9]~log10(P_p2_set)[8:9],col="light green",pch=1,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[2]][8:9]~log10(P_p2_set)[8:9],col="dark green",pch=1,cex=1)
points(Rep_opt_DA_Deg_S40_perm[[3]][1:6]~log10(P_p2_set)[1:6],col="light blue",pch=19,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[3]][1:6]~log10(P_p2_set)[1:6],col="blue",pch=16)
points(Rep_opt_DA_Deg_S40_perm[[3]][7]~log10(P_p2_set)[7],col="light blue",pch=1,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[3]][7]~log10(P_p2_set)[7],col="blue",pch=16)
points(Rep_opt_DA_Deg_S40_perm[[3]][8:9]~log10(P_p2_set)[8:9],col="light blue",pch=1,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[3]][8:9]~log10(P_p2_set)[8:9],col="blue",pch=1,cex=1)
points(Rep_opt_DA_Deg_S40_perm[[4]][1:6]~log10(P_p2_set)[1:6],col="lightsalmon",pch=19,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[4]][1:6]~log10(P_p2_set)[1:6],col="brown",pch=16,cex=1)
points(Rep_opt_DA_Deg_S40_perm[[4]][7:9]~log10(P_p2_set)[7:9],col="lightsalmon",pch=1,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[4]][7:9]~log10(P_p2_set)[7:9],col="brown",pch=1,cex=1)
text(-4.75,1.02,"*",srt=addtxt$srt,font=2,col="black",cex=1)
text(-3.75,1.02,"*",srt=addtxt$srt,font=2,col="black",cex=1)
text(-4.75,0.47,"*",srt=addtxt$srt,font=2,col="black",cex=1)
text(-3.75,0.455,"*",srt=addtxt$srt,font=2,col="black",cex=1)
text(-11.5,1,"A",srt=addtxt$srt,font=2,col="black",cex=1)
par(xpd=TRUE)
...
legend("left",title="Degradation rate",legend=c(expression(paste(eta,"=",10^-3,s^-1)),expression(paste(eta,"=",10^-2.5,s^-1)),expression(paste(eta,"=",10^-2,s^-1)),expression(paste(eta,"=",10^-1.5,s^-1))),bty="y",pch=c(15),col=c("grey","green","blue","red"),ncol=1,cex=0.9)
legend(x=-5.9,y=0.95,legend=c("Stable CF
...
dev.print(device = jpeg, file = "Cross-feeding-deg-comp-efflow-costrans.jpeg", , width = 575*6,height=500*6,res=600,type="cairo")
dev.off()

###3.Influence of degradation rate and fitness contributions -- pathway yield=1:1 -- on the allocation between subpathways (influence of transporters)
...
points(Rep_opt_DA_Deg_S40_perm[[2]][1:7]~log10(P_p2_set)[1:7],col="dark green",pch=16,cex=1)
points(Rep_opt_DA_Deg_S40_perm[[2]][8:9]~log10(P_p2_set)[8:9],col="light green",pch=1,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[2]][8:9]~log10(P_p2_set)[8:9],col="dark green",pch=16,cex=1)
...
points(Rep_opt_DA_Deg_S40_perm[[4]][1:6]~log10(P_p2_set)[1:6],col="lightsalmon",pch=19,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[4]][1:6]~log10(P_p2_set)[1:6],col="brown",pch=16,cex=1)
points(Rep_opt_DA_Deg_S40_perm[[4]][7]~log10(P_p2_set)[7],col="lightsalmon",pch=1,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[4]][7]~log10(P_p2_set)[7],col="brown",pch=16,cex=1)
points(Rep_opt_DA_Deg_S40_perm[[4]][8:9]~log10(P_p2_set)[8:9],col="lightsalmon",pch=1,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[4]][8:9]~log10(P_p2_set)[8:9],col="brown",pch=1,cex=1)
text(-4.75,0.5,"*",srt=addtxt$srt,font=2,col="black",cex=1)
text(-3.75,0.46,"*",srt=addtxt$srt,font=2,col="black",cex=1)
text(-12,1,"B",srt=addtxt$srt,font=2,col="black",cex=1)
par(xpd=TRUE)
legend("bottomleft",title="Singular strat.",legend=c("Loc. stable","Loc. instable","Glob. stable","Glob. instable"),bty="y",pch=c(20,1,19,1),col=c(1,"dark grey",1,1),pt.cex=c(1,1,1.75,1.75),ncol=1,cex=0.9)
legend("left",title="Degradation rate",legend=c(expression(paste(eta,"=",10^-3,s^-1)),expression(paste(eta,"=",10^-2.5,s^-1)),expression(paste(eta,"=",10^-2,s^-1)),expression(paste(eta,"=",10^-1.5,s^-1))),bty="y",pch=c(15),col=c("grey","green","blue","red"),ncol=1,cex=0.9)
legend(x=-5.9,y=0.85,legend=c("Stable CF
coalitions"),bty="n",pch=c("*"),col=c("black"),ncol=1,cex=0.9)
setwd(dir="~")
setwd(dir="/Users/florianlabourel/Desktop/Ongoing-projects/cross-feeding/Proteome allocation/Draft/SM/Figures-February")
dev.print(device = jpeg, file = "Cross-feeding-deg-comp-higheff-costrans.jpeg", , width = 575*6,height=500*6,res=600,type="cairo")
dev.off()
