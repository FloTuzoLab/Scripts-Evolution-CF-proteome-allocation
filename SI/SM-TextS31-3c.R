...
Etot_back_set<-c(10.25e-3,10.5e-3,1.075e-2,1.1e-2,1.125e-2)#5.5*10^-3,8.5e-3,9e-3,9.5e-3,10e-3)#other protein concentration
...
#Variables
Etot_var<-10^seq(-4,-3,length=N_reso)
...
Path1_size=P1s_func(Pts=Path_size,ratio1_2=2/3)
Tox<-Tox_set[1]
  ...
  for (p in 1:length(eta_set)){
    ...
    for (i in 1:N_reso){
      ...
      while((abs(Phi_fit-Phi_eq)/Phi_eq>10^-8) && N_eq>1){
        ...
        S<-nleqslv(c(1e-2,1e-2),transport,jac=NULL)$x
        Sin<-S[2]
        P1<-c()
...
dev.print(device = jpeg, file = "Coexistence-PIP-etamax-Pathlength6.jpeg", width = 1500*6,height=300*6,res=600,type="cairo")
dev.off()
