      ...
      while(r2<(N_reso)){
        ...
        if(list_fit_inv_coex_5_area[[p]][[pe]][r1,r2]==1){
          ...
          while((abs(Phi_fit[1]-Phi_eq)/Phi_eq>10^-6) && (N_eq1>1 || N_eq2>1) && (abs(Phi_fit[2]-Phi_eq)/Phi_eq>10^-6)){
            ...
            Phi_fit[1]<-(Phi1r[1]*PO1+Phi2r[1]*PO2)-prot_cost*(Etot1r1*Path1_size+Etot2r1*(Path_size-Path1_size)+Etot_back)
            Phi_fit[2]<-(Phi1r[2]*PO1+Phi2r[2]*PO2)-prot_cost*(Etot1r2*Path1_size+Etot2r2*(Path_size-Path1_size)+Etot_back)
            if(count<10){
              if(Phi_fit[1]>Phi_eq){
                N_eq1=N_eq1*2
              }
          ...
          Phirr<-(Phi1rr*PO1+Phi2rr*PO2)-prot_cost*(c(Etot1r1,Etot1r2)*Path1_size+c(Etot2r1,Etot2r2)*(Path_size-Path1_size)+(Etot_back))
          ...
          for (rm in 1:max_rm){
            ...
            if(as.numeric((Phi1*PO1+Phi2*PO2)-prot_cost*(E_tot_conc1*Path1_size+E_tot_conc2*(Path_size-Path1_size)+(Etot_back)))>0){
              P_Phi_eq=as.numeric((Phi1*PO1+Phi2*PO2)-prot_cost*(E_tot_conc1*Path1_size+E_tot_conc2*(Path_size-Path1_size)+(Etot_back)))
            }
            else{
              P_Phi_eq=0
            }
            if(rm<3){
              fit=P_Phi_eq/Phi_fit-Phirr[1]/Phi_fit
            }else{
              fit=P_Phi_eq/Phi_fit-Phirr[2]/Phi_fit
            }
...
for (p in 1:length(eta_set)){
  for (pe in 1:1){
    multiplePlot("","","","",c(""),c("")
                 ,ncol=2,f_var_print,f_var_print,list(list_fit_inv_coex_5_area[[p]][[1]]),
