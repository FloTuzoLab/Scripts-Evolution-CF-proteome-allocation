...
#1.Adaptive dynamics - influence of toxicity
...
for(po in 1:length(PayOff_set)){
  ...
  for (to in 1:length(Tox_set)){
    ...
    list_fit_inv_S40_noperm[[to+length(Tox_set)*(po-1)]]<-fit_inv
    tab_N_eq[[to+length(Tox_set)*(po-1)]]<-max(Pop_DA)
  }
}
...
for(po in 1:length(PayOff_set)){
  ...
  for (to in 1:length(Tox_set)){
    i=1
    while(list_fit_inv_S40_noperm[[to+length(Tox_set)*(po-1)]][i,i]==-100 && i<N_reso){
      i=i+1
      if(i==N_reso){
    ...
    S_opt=0
    while (i<N_reso-1){
      if(list_fit_inv_S40_noperm[[to+length(Tox_set)*(po-1)]][i,i+1]<0 && list_fit_inv_S40_noperm[[to+length(Tox_set)*(po-1)]][i,i-1]<0){
        print(list_fit_inv_S40_noperm[[to+length(Tox_set)*(po-1)]][i,i])
        S_opt=i
        print(S_opt)
        Conc1_opt_DA_E0E1comb[[Pox_i]][to]<-Etot_var[S_opt]
        print(Etot_var[S_opt])
        break
      }
      else if (i==N_reso-1){
        if(list_fit_inv_S40_noperm[[to+length(Tox_set)*(po-1)]][i+1,i]<0 && list_fit_inv_S40_noperm[[to+length(Tox_set)*(po-1)]][i,i+1]>0){
          Conc1_opt_DA_E0E1comb[[Pox_i]][p]=10^-4
          i=i+1
          break
        }
        else{
...
for (to in 1:length(Tox_set)){
  list_fit_inv_S40ratvar_Y1_eta1[[to]]<-list()
  Tox<-Tox_set[to]
  PayOff<-PayOff_set[1]
  ratio_12<-ratio_set[1]
  Path1_size=P1s_func(Pts=Path_size,ratio1_2=ratio_12)
  for (pe in 1:length(P_p2_set)){
    Etot_conc=Conc1_opt_DA_E0E1comb[[as.character(PayOff)]][to]*2# Double so that investing half in each sub-pathway coincides with the optimum when considered as a single integrated pathway
    P_p2=P_p2_set[pe]
    eta_p=eta+P_p2*S_c/V_c
    PO1<-5
    PO2<-5
    ...
    list_fit_inv_S40ratvar_Y1_eta1[[to]][[pe]]<-fit_inv
  }
}
...
pal<-list(palet)
Pop_DA1<-Pop_DA
multiplePlot("Main=","","Deg. rate=","",c("1:1","1:2","1:3"),c("1e-4","1e-3","1e-2")
...
Rep_opt_DA_Deg_S40_perm<-list()
for (to in 1:length(Tox_set)){
  Rep_opt_DA_Deg_S40_perm[[to]]<-c(0)
  for (pe in 1:length(P_p2_set)){
    i=1
    while(list_fit_inv_S40ratvar_Y1_eta1[[to]][[pe]][i,i]==-100 && i<N_reso){
      i=i+1
    }
    S_opt=0
    print(i)
    while (i<N_reso){
      #print(i)
      if(i>1 && i<(N_reso-1)){
        if(list_fit_inv_S40ratvar_Y1_eta1[[to]][[pe]][i+1,i]>0 && list_fit_inv_S40ratvar_Y1_eta1[[to]][[pe]][i-1,i]>0){
          print(list_fit_inv_S40ratvar_Y1_eta1[[to]][[pe]][i,i])
          S_opt=i
          print(S_opt)
          Rep_opt_DA_Deg_S40_perm[[to]][pe]<-f_var[S_opt]
          break
        }
      }
      else if (i==(N_reso-1)){
        if(list_fit_inv_S40ratvar_Y1_eta1[[to]][[pe]][i+1,i]<0 && list_fit_inv_S40ratvar_Y1_eta1[[to]][[pe]][i,i+1]>0){
          Rep_opt_DA_Deg_S40_perm[[to]][pe]=1
          i=i+1
          break
        }
        else{
          Rep_opt_DA_Deg_S40_perm[[to]][pe]<-0
          break
        }
      }
      i=i+1
...
plot(Rep_opt_DA_Deg_S40_perm[[3]]~log10(P_p2_set),cex=0.2,col=1,pch=15,ylim=c(-0,1),xlab="log10(Permeability rate)",ylab=expression(paste(delta," (% 1st pathway)")))
points(Rep_opt_DA_Deg_S40_perm[[3]][1:8]~log10(P_p2_set)[1:8],col="light green",pch=19,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[3]][1:8]~log10(P_p2_set)[1:8],col="dark green",pch=16,cex=1)
points(Rep_opt_DA_Deg_S40_perm[[2]][1:8]~log10(P_p2_set)[1:8],col="light blue",pch=19,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[2]][1:8]~log10(P_p2_set)[1:8],col="blue",pch=16)
points(Rep_opt_DA_Deg_S40_perm[[2]][9]~log10(P_p2_set)[9],col="light blue",pch=1,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[2]][9]~log10(P_p2_set)[9],col="blue",pch=16,cex=1)
points(Rep_opt_DA_Deg_S40_perm[[1]][1:7]~log10(P_p2_set)[1:7],col="lightsalmon",pch=19,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[1]][1:7]~log10(P_p2_set)[1:7],col="brown",pch=16)
points(Rep_opt_DA_Deg_S40_perm[[1]][8]~log10(P_p2_set)[8],col="lightsalmon",pch=1,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[1]][8]~log10(P_p2_set)[8],col="brown",pch=16,cex=1)
points(Rep_opt_DA_Deg_S40_perm[[1]][9]~log10(P_p2_set)[9],col="lightsalmon",pch=1,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[1]][9]~log10(P_p2_set)[9],col="brown",pch=1,cex=1)
text(-12,1,"A",srt=addtxt$srt,font=2,col="black",cex=1)
...
legend("topright",title="Toxicity constant",legend=c(expression(paste(T,"=",1,M)),expression(paste(T,"=",10^-1,M)),expression(paste(T,"=",10^-2,M))),bty="y",pch=c(15),col=c("green","blue","red"),ncol=1,cex=0.9)
legend(x=-5.9,y=0.11,legend=c("Stable CF
coalitions"),bty="n",pch=c("*"),col=c("black"),ncol=1,cex=0.9)
setwd(dir="~")
setwd(dir="/Users/florianlabourel/Desktop/Ongoing-projects/cross-feeding/Proteome allocation/Draft/SM/Figures-February")
dev.print(device = jpeg, file = "Cross-feeding-deg-tox.jpeg", width = 575*6,height=500*6,res=600,type="cairo")
dev.off()

###3.Influence of degradation rate and fitness contributions -- pathway yield=1:1 -- on the allocation between subpathways (no influence of transporters)
...
#Parameters
eta_set=c(10^-3,10^-2.5,10^-2,10^-1)
...
for (p in 1:length(eta_set)){
  list_fit_inv_S40ratvar_Y1_eta1[[p]]<-list()
  eta<-eta_set[p]
  PayOff<-PayOff_set[1]
  ratio_12<-ratio_set[1]
  Path1_size=P1s_func(Pts=Path_size,ratio1_2=ratio_12)
  for (pe in 1:length(P_p2_set)){
    ...
    list_fit_inv_S40ratvar_Y1_eta1[[p]][[pe]]<-fit_inv
  }
}
...
Pop_DA1<-Pop_DA
multiplePlot("Main=","","Deg. rate=","",c("1:1","1:2","1:3"),c("1e-4","1e-3","1e-2")
             ,ncol=800,f_var_print,f_var_print,list_fit_inv_S40ratvar_Y1_eta1[[4]],
             abs="res. (% 1st pathway)",ord="mut. (% 1st pathway)",scale=c(-1000,1000),lev=c(0),palette=pal,cextext=1.5,TEXT_to_Add=addtxt,
             image=TRUE,pcex=1,subcex=1,labcex=1.5,axcex=0.75,globcex=0.5,legcex=1,contourlab=TRUE,meth="edge",contcex=0.5,colorkey="COMMON")


Rep_opt_DA_Deg_S40_perm<-list()
for (p in 1:length(eta_set)){
  Rep_opt_DA_Deg_S40_perm[[p]]<-c(0)
  for (pe in 1:length(P_p2_set)){
    i=1
    while(list_fit_inv_S40ratvar_Y1_eta1[[p]][[pe]][i,i]==-100 && i<N_reso){
      i=i+1
    }
    S_opt=0
    print(i)
    while (i<N_reso){
      #print(i)
      if(i>1 && i<(N_reso-1)){
        if(list_fit_inv_S40ratvar_Y1_eta1[[p]][[pe]][i+1,i]>0 && list_fit_inv_S40ratvar_Y1_eta1[[p]][[pe]][i-1,i]>0){
          print(list_fit_inv_S40ratvar_Y1_eta1[[p]][[pe]][i,i])
          S_opt=i
          print(S_opt)
          Rep_opt_DA_Deg_S40_perm[[p]][pe]<-f_var[S_opt]
          break
        }
      }
      else if (i==(N_reso-1)){
        if(list_fit_inv_S40ratvar_Y1_eta1[[p]][[pe]][i+1,i]<0 && list_fit_inv_S40ratvar_Y1_eta1[[p]][[pe]][i,i+1]>0){
          Rep_opt_DA_Deg_S40_perm[[p]][pe]=1
          i=i+1
          break
        }
        else{
          Rep_opt_DA_Deg_S40_perm[[p]][pe]<-0
          break
        }
      }
      i=i+1
...
addtxt<-list(l=-1.6,h=-2.8,txt=c("A","B","C","D","E","F","G","H","I"),srt = 0,font=2,col="black")
par(mfrow=c(1,1),mai=c(1,1,0.5,1))
plot(Rep_opt_DA_Deg_S40_perm[[1]]~log10(P_p2_set),col=1,pch=15,ylim=c(-0,1),xlab="log10(Permeability rate)",ylab=expression(paste(delta," (% 1st pathway)")))
points(Rep_opt_DA_Deg_S40_perm[[1]]~log10(P_p2_set),col="dark grey",pch=19,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[1]]~log10(P_p2_set),col="black",pch=16,cex=1)
points(Rep_opt_DA_Deg_S40_perm[[2]]~log10(P_p2_set),col="light green",pch=19,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[2]]~log10(P_p2_set),col="dark green",pch=16,cex=1)
points(Rep_opt_DA_Deg_S40_perm[[3]][1:7]~log10(P_p2_set)[1:7],col="light blue",pch=19,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[3]][1:7]~log10(P_p2_set)[1:7],col="blue",pch=16)
points(Rep_opt_DA_Deg_S40_perm[[3]][8:9]~log10(P_p2_set)[8:9],col="light blue",pch=1,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[3]][8:9]~log10(P_p2_set)[8:9],col="blue",pch=16,cex=1)
points(Rep_opt_DA_Deg_S40_perm[[4]]~log10(P_p2_set),col="lightsalmon",pch=19,cex=2.5)
points(Rep_opt_DA_Deg_S40_perm[[4]]~log10(P_p2_set),col="brown",pch=16,cex=1)
text(-11.5,1,"B",srt=addtxt$srt,font=2,col="black",cex=1)
par(xpd=TRUE)
legend("bottomleft",title="Singular strat.",legend=c("Loc. stable","Loc. instable","Glob. stable","Glob. instable"),bty="y",pch=c(20,1,19,1),col=c(1,"dark grey",1,1),pt.cex=c(1,1,1.75,1.75),ncol=1,cex=0.9)
legend("bottomright",title="Degradation rate",legend=c(expression(paste(eta,"=",10^-3,s^-1)),expression(paste(eta,"=",10^-2.5,s^-1)),expression(paste(eta,"=",10^-2,s^-1)),expression(paste(eta,"=",10^-1.5,s^-1))),bty="y",pch=c(15),col=c("grey","green","blue","red"),ncol=1,cex=0.9)

setwd(dir="~")
setwd(dir="/Users/florianlabourel/Desktop/Ongoing-projects/cross-feeding/Proteome allocation/Draft/SM/Figures-February")
dev.print(device = jpeg, file = "Cross-feeding-deg-comp-P1.jpeg", , width = 575*6,height=500*6,res=600,type="cairo")
dev.off()
