...
#1.Adaptive dynamics Environment effect
...
beta_set=c(10^-4,10^-5)
alpha_set=c(10^-4,10^-5)
dl=1e-4#dm
V_env=dl^3
...
for(po in 1:length(alpha_set)){
  Pox_i<-as.character(alpha_set[po])
...
for (p in 1:length(eta_set)){
  ...
  for (pe in 1:length(P_p2_set)){
    Etot_conc=Conc1_opt_DA_E0E1comb[[as.character(alpha)]][p]*2# Double so that investing half in each sub-pathway coincides with the optimum when considered as a single integrated pathway
    P_p2=P_p2_set[pe]
    eta_p=eta+P_p2*S_c/V_c
    PO1<-5
    PO2<-5
...
pal<-list(palet)
Pop_DA1<-Pop_DA
multiplePlot("","","P (dm/s)=","",c(""),c("1e-8","1e-7","1e-6","1e-5","1e-4")
             ,ncol=800,f_var_print,f_var_print,list_fit_inv_S40ratvar_Y1_eta1[[2]],
             abs="res. (% 1st pathway)",ord="mut. (% 1st pathway)",scale=c(-99,99),palette=pal,cextext=1.5,TEXT_to_Add=addtxt,
             image=TRUE,pcex=1,subcex=1,labcex=1.5,axcex=0.75,globcex=0.5,legcex=1,contourlab=TRUE,meth="edge",contcex=0.5,colorkey="COMMON")

setwd(dir="~")
setwd(dir="/Users/Florian/Desktop/Manuscripts/Revision-MCF")
dev.print(device = jpeg, file = "Coexistence-PIP-etamax-alpha4.jpeg", , width = 1500*6,height=300*6,res=600,type="cairo")
dev.off()

#B.low rate of environment replenishment (alpha=beta=1e-5)
...
setwd(dir="~")
setwd(dir="/Users/Florian/Desktop/Manuscripts/Revision-MCF")
dev.print(device = jpeg, file = "Coexistence-PIP-etamax-alpha5.jpeg", , width = 1500*6,height=300*6,res=600,type="cairo")
dev.off()
...
###4.Influence of environment concentration (without cells)
...
#5.Adaptive dynamics with 2 sub-pathways - poor environment (alpha/beta=1e-1)
...
setwd(dir="~")
setwd(dir="/Users/Florian/Desktop/Manuscripts/Revision-MCF")
dev.print(device = jpeg, file = "Coexistence-PIP-etamax-alpha45beta3.jpeg", , width = 1500*6,height=300*6,res=600,type="cairo")
dev.off()
